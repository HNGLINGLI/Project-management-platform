...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE engineers SET name = ?, position = ?, age = ?, start_date = ?, salary = ? WHERE $engineer_id);";
    $stmt = $conn->prepare($sql);
    // Bind parameters, added id at the end for the WHERE clause
    $stmt->bind_param("ssidsi", $name, $position, $age, $startDate, $salary, $engineer_id); // Bind $id here
